...
\node[decision, aspect=2,right of = styrning, node distance = 4.5cm, text width = 6em](regleringKlar){Är regleringen klar?};
...
\draw[->](regleringKlar.east) -- node[near start, above]{nej} (sensorData);
\draw[->](sensorData.north) -| ++(0,1.5) node(lowerright){} -| (regleringKlar.north);
\draw[->](regleringKlar.south) -| ++(0,-1.5) -| (nyModul.south);
