...
Huvudmodul & 360 & 258 & -102\\ \hline
Sensormodul & 165 & 184.5 & +19.5\\ \hline
Styrmodul & 155 & 124 & -31 \\ \hline
Datormodul & 75 & 26.5 & -48.5\\ \hline
Integration och tester & 80 & 234 & +154\\ \hline
Övriga aktiviteter & 545 & 570 & +25\\ \hline
Totalt & 1380 & 1397 & +17\\ \hline
...
\begin{tikzpicture}[scale=0.6]
  ...
  \begin{axis}[xlabel={Vecka},ylabel={Ackumulerad tid [h]}, legend pos = north west, width = \textwidth, legend cell align = left,xmin=0,ymin=0]

    \addplot[blue, mark=*, mark indices={2,3,4,5,6,7,8,9}] table[x index= 0, y index = 1, col sep = comma, mark repeat=1, mark phase = 2] {images/data.txt};
    \addlegendentry{Projekterad tid};

    \addplot[red, mark=square*, mark indices={2,3,4,5,6,7,8,9}] table[x index = 2, y index = 3, col sep = comma, mark=*]{images/data.txt};
...
De tre mest besvärliga problemen som påträffades under projektets gång var:
\begin{enumerate}

	\item Glapp i en av 16-pinnarskontakterna som användes till I\textsuperscript{2}C-bussen
	 \begin{enumerate}[label=\alph*)]
	 	\item Hårdvarufel
	 	\item Kommunikationen via I\textsuperscript{2}C-bussen mellan huvud- och styrmodulen var väldigt oberäknelig. Ibland fastnade båda processorerna väldigt länge i kommunikationen.
	 	\item Glappkontakt i ett av kontaktdonen som flatkabeln mellan huvud- och styrmodul satt i.
	 	\item Efter att ha testat nya kablar och att löda om den felaktiga kontaktet utan någon större framgång bestämde vi oss för att vira på en helt ny kontakt istället för den med glapp.
	 	\item 100 timmar felsökning plus att projektet försenades ungefär en vecka
	 \end{enumerate}

	\item Opålitliga mätvärden från lasersensorn
	 \begin{enumerate}[label=\alph*)]
	 	\item Hårdvarufel
	 	\item Lidarn gav inte tillräckligt exakta mätvärden. Dels varierade värdet för ett specifikt avstånd mer än önskat, dels var skillnaden i värde mellan vissa avstånd inkorrekt (40 cm avstånd gav ett värde motsvarande 48 cm, medan 50 cm avstånd gav ett värde motsvarande 52 cm).
	 	\item Lidarn uppfyller inte tillverkarens specificerade prestanda för alla avstånd.
	 	\item Vi la till en reflexsensor och använde oss av den i kombination med Lidarn för ökad redundans i avståndsmätningen.
	 	\item 60-70 timmar
	 \end{enumerate}
	
	 \item Regleringen på raksträcka reagerade felaktigt på inverterade hörn 
	 \begin{enumerate}[label=\alph*)]
	 	\item Mjukvarufel
	 	\item På väg in i vissa korsningar, det vill säga när roboten passerade ett inverterat hörn, ryckte den ibland till och började följa väggen runt hörnet.
	 	\item Felet berodde på att sidosensorerna ibland mottog reflektioner runt hörnet vilket gjorde att regleringen försökte reglera in mot hörnet.
	 	\item Vi begränsade avståndssensorernas maxavstånd i reglerloopen.
	 	\item 40 timmar
	 \end{enumerate}
...
\begin{enumerate}
  	\item LCD:n fungerade redan efter första veckan (lång före planerat). Detta var väldigt tacksamt de kommande veckorna då det förenklade de kommande veckornas testning avsevärt.
    	\item Datormodulen färdigställdes också före tidplanen. Dess funktionalitet låg alltid steget före robotens, vilket gjorde att nya funktioner hos roboten snabbt kunde testas via datormodulen.
...
Leveransen bestod av presentation av projektet och av en tävling mot andra grupper som genomfört samma projekt. Presentationen var lyckad och och vi erhöll fin feedback från både beställare och opponenter. En andraplats vanns på tävlingen och Pigbot var den enda roboten som lyckades uppfylla alla krav på tävlingsdagen.
