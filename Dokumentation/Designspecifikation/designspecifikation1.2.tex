...
1.1 & & Fjärde utkastet & Grupp 4 & \\ \hline
...
\FloatBarrier
\subsection{Sensorer}
Roboten kommer vara utrustad med sju stycken sensorer. Fyra infraröda sensorer av typen \verb+GP2D120+ används för att bestämma avstånden till väggarna på robotens sidor och robotens vinkel i förhållande till dessa. Vidare finns en vinkelhastighetsgivare, \verb+MLX90609+ som används för att reglera rotation av roboten. Det finns även en lasersensor av typen \verb+Lidar-Lite v2+ för att bestämma avståndet till väggar framför roboten. Den sistnämnda är placerad på ett servo för att kunna roteras. Placeringen av sensorerna visas nedan i figur \ref{sensor}. Slutligen används en reflexsensor för att beräkna tillryggalagd sträcka genom att beräkna antalet rotationer av hjulen.
...
På ett av chassits hjul placeras en skiva bestående av 16 stycken svarta och vita sektioner. En reflexsensor placeras mot skivan och mäter antalet flankar mellan sektionerna som passerar. Denna information används sedan av styrmodulen för att beräkna tillryggalagd sträcka.
...
\begin{itemize}
\item{Processorenhet, \verb+ATmega1284p+ x 1}
...
Sensormodulens processorenhet, \verb+ATmega1284p+, är ansluten med en I\textsuperscript{2}C-buss. Den har som uppgift att kommunicera med huvudmodulen, där sensormodulens processorenheter agerar som slavenhet i förhållande till huvudmodulens dito. 
...
Insamlat sensordata bearbetas i modulens processorenhet för att därefter kommuniceras till huvudmodulen. Figur \ref{kopplingsschema:sensormodul} illustrerar \verb+ATmega1284p+ och dess ingångar, samt hur respektive sensorenhet ska kopplas samman med processorn. 
...
Reflexsensorn används för att beräkna antal varv som robotens hjul roterar. Då skivan som placeras på hjulet består av 16 stycken sektioner kan tillryggalagd sträcka bestämmas med en precision på $\frac{\pi \diameter} {16}$ där $\diameter$ är diametern på hjulet. Sensorn kopplas in till en ADC-ingång hos processorn.
...
\FloatBarrier
\subsubsection{Laser-sensor}
Då lasersensorn \verb+LIDAR-lite v2+ kommer att kopplas samman med processorenheten via ett PWM-interface så kommer avbrott, vilket kallas med hjälp av en timer (\verb+TCNT1+), att behövas. För att göra detta så krävs det att \verb+TIMSK+-registret (Timer/CounterInterrupt Mask Register) initeras. \verb+TIMER1+ kommer att användas vilken är ett 16-bitars timer. Figur \ref{Avbrott_PWM} ger en översiktlig bild av hur PWM-interfacet är tänkt fungera.
\begin{figure}[htbp]
\centering
\noindent\resizebox{.5\linewidth}{!}{
...
Tillsammans med kommandona för rörelse skickas även ett tal som anger med vilken hastighet (i procent av maxhastighet) förflyttningen ska utföras. Ett kommando kan exempelvis se ut såhär: \textit{fram 80}, varpå roboten antingen kör framåt med 80\% av maxhastighet. Modulen kommer vara designad så att ett körkommando gäller tills dess att ett nytt kommando skickas, varför även kommandot för \textit{stopp} är nödvändigt.
...
\end{itemize}
Dessa sensorvärden kommer skickas med en frekvens om 30 Hz förutsatt att reglering är aktiverad, det vill säga vid autonom styrning av roboten.
...
Då roboten navigerar i autonomnt läge består kommunikationen även av sensordata från huvudmodulen. Här kallas en metod som läser in sensordata och lagrar dessa i lokala variabler. Dessa variabler används sedan i regleringen och på så sätt sker alltid regleringen utifrån de allra senaste sensorvärdena. 
...
Panelen för robotstatus visar robotens nuvarande status, gällande körläge, senaste styrkommando med mera. Tabell \ref{datormodul:combinations} visar möjliga knappkombinationer och dess översättning till styrkommando.
...
\begin{itemize}
	...
	\item C (1 byte) - Hastighet (tillåtna värden: 0-245).
\end{itemize}
...
\begin{itemize}
	...
	\item B (1 byte) - Motsvarar nodens x-koordinat (tillåtna värden: 0-30).
	\item C (1 byte) - Motsvarar nodens y-koordinat (tillåtna värden: 0-17).
	\item D (1 byte) - Representerar nodens värde (avstånd från start) med speciella värden för startnoden, målnoden och väggnoder.
\end{itemize}
...
\begin{itemize}
	...
	\item C (1 byte) - 0/1 om av/på. Annars heltal (tillåtna värden: 0-245).
\end{itemize}
...
4 & Lasersensor (high) & cm  \\ \hline
5 & Lasersensor (low) & cm  \\ \hline
6 & Rotation kring z-axeln & rad/s \\ \hline
7 & IR-sensor för mål & cm \\ \hline
8 & Reflexsensor & flanker  \\ \hline
...
4 & K-värde \\ \hline  
5 & Hastighet \\ \hline
6 & Nästa styrbeslut (debug) \\ \hline
7 & Debugläge av/på \\ \hline
8 & Ping \\ \hline
9 & 90-graders rotation \\ \hline
10 & 180-graders rotation \\ \hline
11 & Gripklo öppna/stäng \\ \hline
12 & Senaste kontrollkommandot \\ \hline
13 & Nuvarande omgång \\ \hline
14 & Rotationshastighet \\ \hline
15 & Starta omgång \\ \hline
