...
\node[block, below of = start, node distance = 3cm](initTIMSK){Initiera TIMSK};
\node[block, below of = initTIMSK, node distance = 3cm](TIMER){Initierar TIMER0 med ett värde};
\node[decision, below of = TIMER, node distance = 3cm](int2){TIMER0 overflow};
...
\draw[->](start) -- (initTIMSK);
\draw[->](initTIMSK)-- (TIMER);
\draw[->](TIMER)--(int2);
