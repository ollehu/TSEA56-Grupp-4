...
\begin{tikzpicture}[scale=0.8,rotate=-80]
	...
	\draw[thick, draw=white] 	(6.25,0)  -- (6.25,-5);
	%\draw[thick, draw=black, <->](-.2, -4) -- (3,-4) node[left] {$2e$} --(6.2,-4);
	
	%Speed vector
	\draw[thick, draw=black, dotted, ->] (center) --+ (90:10) node[above,near end]{s};
	...
	%Omega
	\draw[thick,->] ($(center)+(-0.7,0)$) arc (180:380:0.7) node[near start,left]{$\omega$};
