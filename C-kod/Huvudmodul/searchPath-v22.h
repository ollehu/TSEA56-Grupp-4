...
uint8_t targetInCorner()
{
	...
	if (map[target[0]+1][target[1]] == wallValue){
		counter = counter + 1;
	}
	if (map[target[0]-1][target[1]] == wallValue){
		counter = counter + 1;
	}
	if (map[target[0]][target[1]+1] == wallValue){
		counter = counter + 1;
	}
	if (map[target[0]][target[1]-1] == wallValue){
		counter = counter + 1;
...
void updateCoordinates() //Updates map, path and position variables
{
	...
	if (direction == 0){
		...
		if (map[position[0]][position[1]+1] == initialValue){
			...
			} else if ((map[position[0]][position[1]] == targetValue) && targetInCorner()){
				map[position[0]][position[1]+1] = target[2] + 1;
			} else{
		...
		if (map[position[0]+1][position[1]] == initialValue){
			if (map[position[0]][position[1]] == startPositionValue[0]){
				...
				map[position[0]+1][position[1]] = target[2] + 1;
			} else {
	...
	if((map[position[0]+1][position[1]] == map[position[0]][position[1]] - 1) || 
		...
		switch (direction){// 0: north, 1: east, 2: south, 3: west
			...
			case 3:
				...
				(map[position[0]][position[1]+1] == startPositionValue[0]) || 
				((map[position[0]][position[1]+1] == targetValue)  && (target[2] == map[position[0]][position[1]] - 1)) || 
...
uint8_t * shortestWayToStart()
{
	...
	switch(direction){
		case 0:
		if((map[position[0]+1][position[1]] < currentCoordinateValue) || (map[position[0]+1][position[1]] == startPositionValue[0])){
			if(map[position[0]+1][position[1]] == startPositionValue[0]){
				currentCoordinateValue = 0;
			} else {
		...
		if((map[position[0]][position[1]+1] < currentCoordinateValue) || (map[position[0]][position[1]+1] == startPositionValue[0])){
			if(map[position[0]][position[1]+1] == startPositionValue[0]){
				currentCoordinateValue = 0;
				} else {
...
void explore(void)
{
	...
	if((lastCommand[1] == right) || (lastCommand[1] == left) || (lastCommand[1] == oneEighty)) {
		...
		if((lastCommand[2] == 0xB4) && (deadEnd == 0) && (hasFoundTarget() == 0)){
			hasFoundWayBack = 1;
		}
	} else {
		updateCoordinates();
