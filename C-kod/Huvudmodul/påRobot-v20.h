...
void updateCoordinates() //Updates map, path and position variables
{
	...
	if (direction == 0){
		...
		if (map[position[0]][position[1]+1] == initialValue){
			...
			} else if (map[position[0]][position[1]] == targetValue) {
...
{	
	...
	} else if((map[position[0]][position[1]+1] == map[position[0]][position[1]] - 1) || (map[position[0]][position[1]+1] == startPositionValue[0]) || (map[position[0]][position[1]+1] == targetValue)){
		switch (direction){// 0: north, 1: east, 2: south, 3: west
			case 0: 
				//One forward
				return oneForward;
	...
	} else if((map[position[0]-1][position[1]] == map[position[0]][position[1]] - 1) || (map[position[0]-1][position[1]] == startPositionValue[0]) || (map[position[0]-1][position[1]] == targetValue)){
		switch (direction){// 0: north, 1: east, 2: south, 3: west
			case 0:
				//Rotate left
				return rotateLeft;
